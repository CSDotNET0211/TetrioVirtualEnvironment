	...
	public class JudgeSystem : ISystem
	{
		internal static string? IsTspin(GameData gameData)
		{
			...
			for (int index = 0; index < 4; index++)
			{
				...
				minoCorner = ConstData.CORNER_TABLE[GetIndex(gameData.Falling.Type)];
		...
		internal static bool IsEmptyPos(int x, int y, IReadOnlyList<int> field)
		{
		...
		internal static void IsBoardEmpty(GameData gameData)
		{
			...
			for (int y = FIELD_HEIGHT - 1; y >= 0; y--)
			{
				...
				{
					if (gameData.Board[x + y * 10] != (int)MinoKind.Empty)
						return;
