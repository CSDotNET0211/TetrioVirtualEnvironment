...
namespace TetrReplayLoader
{
	...
	public class ReplayLoader
	{
		...
		public static List<Event>? GetReplayEvents(object replayData, ReplayKind replayKind, int playerIndex, int replayIndex)
		{
			if (replayKind == ReplayKind.TTR)
		...
		public static Stats GetReplayStats(object replayData, ReplayKind replayKind, int playerIndex, int replayIndex)
		{
			...
			else
			{
				...
				result.VS = eventEnd.export.aggregatestats.vsscore ?? -1;
				result.APM = eventEnd.export.aggregatestats.apm ?? -1;
				result.PPS = eventEnd.export.aggregatestats.pps ?? -1;

				var frames = GetGameTotalFrames(replayData, replayKind, replayIndex);
				int time = frames / 60;
		...
		public static int GetGameTotalFrames(object replaydata, ReplayKind replayKind, int replayIndex)
		{
			//NotImplemented
			if (replayKind == ReplayKind.TTR)
				return -1;
